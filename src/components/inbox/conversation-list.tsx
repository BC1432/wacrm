...
// `labelKey`s are resolved with `t()` at render time; `value`s are the
// DB-facing filter/status identifiers and must stay untranslated.
...
export function ConversationList({
  ...
  const [channelFilter, setChannelFilter] = useState<MessagingChannel | 'all'>(
    'all'
  );
  ...
  const availableChannels = useMemo(() => {
    return Array.from(
      new Set(
        conversations.map((conversation) => conversation.channel ?? 'whatsapp')
      )
    ).sort();
  }, [conversations]);
        ...
        <div className="flex flex-wrap items-center gap-1">
          <DropdownMenu>
            <DropdownMenuTrigger className="text-muted-foreground hover:text-foreground hover:bg-muted inline-flex h-7 items-center justify-center gap-1 rounded-md px-2 text-xs">
              {t(activeFilter?.labelKey ?? 'inbox.filter.all')}
      ...
      <div className="min-w-0 flex-1">
        ...
        <div className="mt-0.5 flex items-center justify-between gap-2">
          <p className="text-muted-foreground truncate text-xs">
            {conversation.last_message_text || t('inbox.thread.noMessages')}
          </p>
