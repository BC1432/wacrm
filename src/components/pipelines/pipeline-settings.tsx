...
export function PipelineSettings({
  ...
  async function handleSave() {
    ...
    if (renameRes.error || stagesRes.error) {
      toast.error(t("pipelines.settings.toast.saveFailed"));
      return;
    }
    ...
    onPipelinesChanged();
    onStagesChanged();
    toast.success(t("pipelines.settings.toast.saved"));
  }
  ...
  async function handleAddStage() {
    ...
    const { data, error } = await supabase
      ...
      .single();
    if (error || !data) {
      toast.error(t("pipelines.settings.toast.addStageFailed"));
      return;
    }
  ...
  async function handleRemoveStage(stageId: string) {
    ...
    const { count } = await supabase
      ...
      .eq("stage_id", stageId);
    if (count && count > 0) {
      toast.error(t("pipelines.settings.toast.stageHasDeals"));
      return;
    }
    const { error } = await supabase
      ...
      .eq("id", stageId);
    if (error) {
      toast.error(t("pipelines.settings.toast.deleteStageFailed"));
      return;
    }
  ...
  async function handleDeletePipeline() {
    ...
    setDeleting(false);
    if (error) {
      toast.error(t("pipelines.settings.toast.deleteFailed"));
      return;
    }
    onOpenChange(false);
    onPipelinesChanged();
    toast.success(t("pipelines.settings.toast.deleted"));
  }
            ...
            <div className="flex items-center gap-3 rounded-lg border border-red-500/30 bg-red-500/10 p-4">
              ...
              <div>
                <p className="text-sm font-medium text-red-400">
                  {t("pipelines.settings.deleteTitle")}
                </p>
                <p className="mt-1 text-xs text-muted-foreground">
                  {t("pipelines.settings.deleteDescription")}
                </p>
              </div>
            </div>
              ...
              <div className="grid gap-2">
                    ...
                    <div className="space-y-2">
                      {localStages.map((stage, index) => (
                        <SortableStageRow
                          ...
                          colors={STAGE_COLORS}
                          dragLabel={t("pipelines.settings.dragToReorder")}
                          changeColorLabel={t("pipelines.settings.changeColor")}
                        />
                      ))}
                    </div>
                  </SortableContext>
                ...
                <div className="mt-1 flex flex-wrap gap-1">
                  {STAGE_COLORS.map((color) => (
                    <button
                      ...
                      aria-label={t("pipelines.settings.pickColor", { color })}
                    />
                  ))}
                </div>
                <div className="flex items-center gap-2">
                  <Input
                    ...
                    placeholder={t("pipelines.settings.newStageName")}
