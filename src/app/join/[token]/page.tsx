...
// DB role values → i18n keys, resolved with `t()` at render time.
const ROLE_LABEL_KEY: Record<PeekOk['role'], string> = {
  admin: 'join.role.admin',
  agent: 'join.role.agent',
  viewer: 'join.role.viewer',
};

// Peek failure reasons → i18n keys, resolved with `t()` at render time.
...
export default function JoinPage() {
  ...
  const handleAccept = useCallback(async () => {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        if (res.status === 409) {
          setConflictMessage(payload.error || t('join.conflictFallback'));
        } else {
          toast.error(payload.error || t('join.acceptFailed'));
        }
        setAccepting(false);
        return;
      }
      toast.success(t('join.welcomeToTeam'));
      // Full reload (not router.push) so AuthProvider re-fetches
      // the profile with the new account_id and account_role.
      window.location.href = '/dashboard';
    } catch (err) {
      console.error('[join] redeem error:', err);
      toast.error(t('join.serverUnreachable'));
      setAccepting(false);
    }
  }, [token, t]);
  ...
  const handleSignOutAndRetry = useCallback(async () => {
    ...
    try {
      ...
      toast.error(t('join.signOutFailed'));
      setSigningOut(false);
    }
  }, [t]);
          ...
          {peek.reason === 'server_error' ? (
            <>
              ...
              <Link href="/signup">
                ...
                >
                  {t('join.createNewAccountInstead')}
                </Button>
              </Link>
            </>
          ) : (
        ...
        <Card className="w-full max-w-md border-border bg-card">
          ...
          <CardContent className="flex flex-col gap-3">
            ...
            <p className="text-center text-xs text-muted-foreground">
              {t('join.acceptNote', { account: peek.account_name })}
            </p>
          </CardContent>
        </Card>
          ...
          <DialogContent className="bg-popover border-border sm:max-w-md">
            <DialogHeader>
              <DialogTitle className="flex items-center gap-2 text-popover-foreground">
                ...
                {t('join.conflictTitle', { account: peek.account_name })}
              </DialogTitle>
